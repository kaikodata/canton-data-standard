...
- Producer-signed `AuditRecord` templates, one per verifier, created by a successful verify choice when the producer's `allowsAuditRecords` view policy and the caller's `createAuditRecord` argument both opt in; they live in four template-only audit packages and share the `VerificationAudit` evidence type added to `utils-v1`.
