...
A successful verification can also leave a durable, producer-signed
`AuditRecord` on the ledger: the standard's receipt that the verification
happened, visible to the producer and to the verifying party. The record is
written only when both sides opt in, the producer through the
`allowsAuditRecords` policy on its verifier and the caller through the
`createAuditRecord` choice argument.
...
| `interfaces/canton-data-standard-quote-verifier-v1` | `canton-data-standard-quote-verifier-v1` | The `QuoteVerifier` interface: a long-lived contract that holds a producer's secp256k1 public key and authenticates off-ledger-signed quotes through the nonconsuming `QuoteVerifier_Verify` choice. A successful verification writes a producer-signed `AuditRecord` when the producer's `allowsAuditRecords` policy and the caller's `createAuditRecord` request both opt in; the verifier itself is never mutated, so it stays contention-free. |
| `interfaces/canton-data-standard-quote-verifier-audit-v1` | `canton-data-standard-quote-verifier-audit-v1` | The `AuditRecord` template `QuoteVerifier_Verify` creates when both sides opt in: the durable, producer-signed trace of a free quote verification, observed by the verifying party, carrying everything needed to re-check the signature off-ledger. Template-only, so it stays smart-contract-upgradable. |
| `interfaces/canton-data-standard-paid-quote-verifier-v1` | `canton-data-standard-paid-quote-verifier-v1` | The `PaidQuoteVerifier` interface: the pay-as-you-go sibling of `QuoteVerifier`. `PaidQuoteVerifier_VerifyAndPay` authenticates a quote and settles a producer-signed per-call fee, with a single Canton Token Standard transfer, in one transaction, and, when both sides opt in, writes a producer-signed `AuditRecord` that captures the settled payment. |
| `interfaces/canton-data-standard-paid-quote-verifier-audit-v1` | `canton-data-standard-paid-quote-verifier-audit-v1` | The `AuditRecord` template `PaidQuoteVerifier_VerifyAndPay` creates when both sides opt in: the paid sibling of the quote audit record, extended with the fee, instrument, and payee that settled. Template-only, so it stays smart-contract-upgradable. |
...
| `examples/paid-verifier-consumer` | `paid-verifier-consumer-example` | A reference consumer for the paid path: a trade workflow that authenticates a pulled quote through a disclosed `PaidQuoteVerifier` and pays the per-call fee. Settles into its own `PaidVerifiedQuoteTradeSettlement` record, which also carries the settled fee. |
| `interfaces/canton-data-standard-datapoint-verifier-v1` | `canton-data-standard-datapoint-verifier-v1` | The `DataPointVerifier` interface: the data point sibling of `QuoteVerifier`. A long-lived contract that holds a producer's secp256k1 public key and authenticates off-ledger-signed data points through the nonconsuming `DataPointVerifier_Verify` choice, under the recursive `v1-datapoint-tlv` canonical encoding. A successful verification writes a producer-signed `AuditRecord` when the producer's `allowsAuditRecords` policy and the caller's `createAuditRecord` request both opt in; the verifier itself is never mutated. |
| `interfaces/canton-data-standard-datapoint-verifier-audit-v1` | `canton-data-standard-datapoint-verifier-audit-v1` | The `AuditRecord` template `DataPointVerifier_Verify` creates when both sides opt in: the durable, producer-signed trace of a free data point verification, observed by the verifying party, carrying the schema, payload, and evidence needed to re-check the signature off-ledger. Template-only, so it stays smart-contract-upgradable. |
...
| `interfaces/canton-data-standard-paid-datapoint-verifier-audit-v1` | `canton-data-standard-paid-datapoint-verifier-audit-v1` | The `AuditRecord` template `PaidDataPointVerifier_VerifyAndPay` creates when both sides opt in: the paid sibling of the data point audit record, extended with the fee, instrument, and payee that settled. Template-only, so it stays smart-contract-upgradable. |
...
| `examples/paid-datapoint-verifier-consumer` | `paid-datapoint-verifier-consumer-example` | A reference consumer for the paid path: a `PaidRateSubscription` that authenticates a pulled data point through a disclosed `PaidDataPointVerifier` and pays the per-call fee. Records into its own `PaidRecordedRate` record, which also carries the settled fee. |
